	...
	public ILista<T> unificar(Comparator<T> comparador) {
		...
		try
		{
			for(int i = 1; i <= this.size(); i++)
			{
				...
				T siguiente = this.getElement(i+1);

				T compare = siguiente != null ? siguiente : this.getElement(i-1);

				if(compare != null && comparador.compare(actual, compare) == 0)
					continue;
